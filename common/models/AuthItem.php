...
class AuthItem extends \yii\db\ActiveRecord
{
    ...
    public function rules()
    {
        return [
            ...
            ['type', 'default', 'value' => Constants::TYPE_ROLE], // По умолчанию тип записи "Роль"
        ];
    }
    ...
    public static function getPermissionCompanies()
    {
        $model = self::find()
            ->where(['type' => Constants::TYPE_PERMISSION])
            ->andWhere(['place' => Constants::ITEM_PERMISSION_COMPANY])
            ->orderBy('description')
            ->all();
    ...
    public static function getPermissionTariffs()
    {
        $model = self::find()
            ->where(['type' => Constants::TYPE_PERMISSION])
            ->andWhere(['place' => Constants::ITEM_PERMISSION_TARIFF])
            ->orderBy('description')
            ->all();
