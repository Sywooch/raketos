        ...
        if ($model->validate()):
            ...
            try {
                if ($modelPhoto->save() && $commit):
                    FileHelper::createDirectory(\Yii::getAlias($this->imageData['baseUrl'])
                        . $this->imageData['imagePath'] . $md5_1 . '/' . $md5_2 . '/', $mode = 509);
                    if ($model->image->saveAs(\Yii::getAlias($this->imageData['baseUrl'])
                        . $this->imageData['imagePath'] . $md5_1 . '/' . $md5_2 . '/' . $fileName)
                    ):
                        $image = Image::getImagine();
                        $newImage = $image->open(\Yii::getAlias($this->imageData['baseUrl'])
                            . $this->imageData['imagePath'] . $md5_1 . '/' . $md5_2 . '/' . $fileName);
                        $newImage->rotate($paramsCrop['rotate']);

                        $paramsCrop['x'] = ($paramsCrop['x'] > 0)
                            ? $paramsCrop['x']
                            : 0;
                        $paramsCrop['y'] = ($paramsCrop['y'] > 0)
                            ? $paramsCrop['y']
                            : 0;
                        if ($newImage->crop(new Point($paramsCrop['x'], $paramsCrop['y']),
                            new Box($paramsCrop['width'], $paramsCrop['height']))
                            ->save(\Yii::getAlias($this->imageData['baseUrl']) . $this->imageData['imagePath'] . $md5_1
                                . '/' . $md5_2 . '/' . $fileName)
                        ):

                            $newImage = $image->open(\Yii::getAlias($this->imageData['baseUrl'])
                                . $this->imageData['imagePath'] . $md5_1 . '/' . $md5_2 . '/' . $fileName);
                            $newImage->thumbnail(new Box($this->imageData['imageSmallWidth'],
                                $this->imageData['imageSmallHeight']))
                                ->save(\Yii::getAlias($this->imageData['baseUrl']) . $this->imageData['imagePath']
                                    . $md5_1 . '/' . $md5_2 . '/' . $smallFileName);
                            $transaction->commit();
                            $this->overlayWatermark(\Yii::getAlias($this->imageData['baseUrl']) . $modelPhoto->file);
                            $this->overlayWatermark(\Yii::getAlias($this->imageData['baseUrl'])
                                . $modelPhoto->file_small);
                        endif;
                    endif;
    ...
    public function updateImage()
    {
        ...
        if ($model->validate()):
            ...
            try {
                ...
                $this->deleteImageFile(\Yii::getAlias($this->imageData['baseUrl']) . $this->imageData['imagePath']
                    . $md5_1 . '/' . $md5_2 . '/' . $fileName);
                $this->deleteImageFile(\Yii::getAlias($this->imageData['baseUrl']) . $this->imageData['imagePath']
                    . $md5_1 . '/' . $md5_2 . '/' . $smallFileName);
                $transaction->rollBack();
            }
